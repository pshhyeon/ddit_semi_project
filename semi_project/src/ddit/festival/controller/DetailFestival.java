...
public class DetailFestival extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println(fesVo);
		
		Gson gson = new Gson();
		String detailFes = gson.toJson(fesVo);
		System.out.println(detailFes);
		request.setAttribute("detailFestival", detailFes);
		//response.getWriter().write(detailFes);
		request.getRequestDispatcher("/festival/detailFestival.jsp").forward(request, response);
	}
